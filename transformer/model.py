...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            y = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=self.dropout, is_causal=True) * (1.0 / math.sqrt(k.size(-1))) #muP addition
        else:
            # manual implementation of attention
            att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1))) * (1.0 / math.sqrt(k.size(-1))) #muP addition
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.c_fc    = nn.Linear(config.n_embd, 4 * config.n_embd, bias=config.bias) # chris 1 was 4
        self.c_proj  = nn.Linear(4 * config.n_embd, config.n_embd, bias=config.bias) # chris 1 was 4
