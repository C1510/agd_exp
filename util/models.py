...
def getNetwork(args, input_dim, output_dim, bias=True, affine=True):
    if args.arch == 'fcn':
        return FCN(args.depth, args.width, input_dim, output_dim, bias=True)
    elif 'imagenet' in args.dataset and args.arch == 'resnet50':
        return resnet50(num_classes=1000, bias=bias, affine=affine)
    elif 'cifar' not in args.dataset:
        raise Exception("That network only works with CIFAR.")
    elif args.arch == 'vgg':
        return VGG16(output_dim, bias=bias, affine=affine)
    elif args.arch == 'resnet18':
        return PreActResNet18(output_dim, bias=bias, affine=affine)
    elif args.arch == 'resnet50':
        return PreActResNet50(output_dim)

def PreActResNet18(output_dim, bias=False, affine=False):  return PreActResNet(PreActBlock,      [2,2,2,2],  output_dim, bias=bias, affine=affine)
def PreActResNet50(output_dim, bias=False, affine=False):  return PreActResNet(PreActBottleneck, [3,4,6,3],  output_dim, bias=bias, affine=affine)

def VGG16(output_dim, bias=False, affine=False): return VGG_CIFAR([64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M', 512, 512, 512, 'M'], output_dim, bias=bias, affine=affine)
...
class PreActBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, bias=False, affine=False):
...
class PreActResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, bias=False, affine=False):
        super(PreActResNet, self).__init__()
        self.in_planes = 64

        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=bias)
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1, bias=bias, affine=affine)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2, bias=bias, affine=affine)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2, bias=bias, affine=affine)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2, bias=bias, affine=affine)
        self.linear = nn.Linear(512*block.expansion, num_classes, bias=bias)

    def _make_layer(self, block, planes, num_blocks, stride, bias, affine):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, bias, affine))
